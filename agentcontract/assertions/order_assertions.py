...
def assert_before(trace: AgentTrace, tool: str, before_tool: str) -> None:
    """Assert that tool was called before another tool."""
    my_idx = trace.index_of(tool)
    other_idx = trace.index_of(before_tool)
...
def assert_after(trace: AgentTrace, tool: str, after_tool: str) -> None:
    """Assert that tool was called after another tool."""
    my_idx = trace.index_of(tool)
    other_idx = trace.index_of(after_tool)
...
def assert_immediately_after(trace: AgentTrace, tool: str, after_tool: str) -> None:
