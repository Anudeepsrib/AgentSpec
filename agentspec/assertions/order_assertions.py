...
def assert_before(
    ...
    if my_idx >= other_idx:
        raise OrderViolation(
            f"'{tool}' (step {my_idx}) should be before '{before_tool}' (step {other_idx})"
        )
...
def assert_after(
    ...
    if my_idx <= other_idx:
        raise OrderViolation(
            f"'{tool}' (step {my_idx}) should be after '{after_tool}' (step {other_idx})"
        )


def assert_immediately_after(
    trace: AgentTrace,
    tool: str,
    after_tool: str,
    my_agent_id: str | None = None,
